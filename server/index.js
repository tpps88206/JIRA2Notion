  ...
  try {
    for (const user of Firestore.Users) {
      ...
      for (const issue of needAddItems) {
        addIssue(notion, user.notionDatabaseId, issue.fields.status.name === 'Done', issue.fields.status.name ,issue.fields.summary, issue.key, `https://17media.atlassian.net/browse/${issue.key}`);
      }
